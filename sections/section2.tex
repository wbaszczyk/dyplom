...
Schemat układu urządzenia przedstawia \autoref{fig:schemat_pcb}. Mamy tutaj przedstawione urządzenie \texttt{ESP8266} (Component 1), piny \texttt{Reset}, \texttt{CHPD} oraz \texttt{VCC} podpięte są do napięcia wysokiego 3,3V. Piny ozaczone jako \texttt{T1}, \texttt{T2}, \texttt{T3} to wyprowadzenia do tranzystorów, które sterują kolorami diod LED RGB. Piny \texttt{{RXD} oraz \texttt{TXD} służą do wgrywania programu, wykonuje się to komendą 
\begin{lstlisting}
